...
```
node "${COPILOT_CLI_PLUGIN_ROOT}/skills/rad-orchestration/scripts/radorch.mjs" plan resolve --project <project_name> [--template <project_template>]
```
...
- Dispatch a **`general-purpose`** subagent to audit the Requirements doc and the Master Plan. Give the subagent both doc paths and instruct it to follow `${COPILOT_CLI_PLUGIN_ROOT}/skills/rad-plan/references/audit.md`. The subagent returns a structured report with frontmatter `verdict: approved` or `verdict: issues_found`. The auditor does NOT edit either planning doc — it reports.

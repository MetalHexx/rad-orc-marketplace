...
```
node "${COPILOT_VSCODE_PLUGIN_ROOT}/skills/rad-orchestration/scripts/radorch.mjs" worktree remove --project <name>
```

To remove a single repo's worktree (targeted recovery or corruption fix):

```
node "${COPILOT_VSCODE_PLUGIN_ROOT}/skills/rad-orchestration/scripts/radorch.mjs" worktree remove --project <name> --repo <repo>
```
